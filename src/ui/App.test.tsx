...
describe('fluxo de etapas', () => {
  ...
  it('pula destinatÃ¡rios quando nenhum pagante enviar e-mail', async () => {
    const { App } = await import('./App')
    render(<App />)
    await screen.findByRole('heading', { name: /quem vai pagar/i })
    fireEvent.click(screen.getByRole('checkbox', { name: /selecionar carlos mendes/i }))
    fireEvent.click(screen.getByRole('button', { name: /^continuar$/i }))
    const emailSwitch = await screen.findByRole('checkbox', { name: /e-mail/i })
    fireEvent.click(emailSwitch)
    fireEvent.click(screen.getByRole('button', { name: /^continuar$/i }))
    expect(screen.queryByRole('heading', { name: /confirme o destinatÃ¡rio do e-mail/i })).toBeNull()
    expect(screen.getByRole('button', { name: /gerar pagantes/i }).getAttribute('aria-disabled')).toBe('false')
    fireEvent.click(screen.getByRole('button', { name: /gerar pagantes/i }))
    await waitFor(() => expect(mocks.submitOperation).toHaveBeenCalledOnce())
  })
})
